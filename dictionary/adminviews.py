...
class GlossListView(ListView):
    ...
    def get_queryset(self):
        ...
        if 'inWeb' in get and get['inWeb'] != 'unspecified':
            val = get['inWeb'] == 'yes'
        ...
        if 'domhndsh' in get and get['domhndsh'] != '':
            val = get['domhndsh']
            qs = qs.filter(domhndsh__exact=val)

            #print "C :", len(qs)

        if 'subhndsh' in get and get['subhndsh'] != '':
            val = get['subhndsh']
            qs = qs.filter(subhndsh__exact=val)
            #print "D :", len(qs)

        if 'final_domhndsh' in get and get['final_domhndsh'] != '':
            val = get['final_domhndsh']
            qs = qs.filter(final_domhndsh__exact=val)
            #print "E :", len(qs)

        if 'final_subhndsh' in get and get['final_subhndsh'] != '':
            val = get['final_subhndsh']
            qs = qs.filter(final_subhndsh__exact=val)
           # print "F :", len(qs)

        if 'locprim' in get and get['locprim'] != '':
            val = get['locprim']
            qs = qs.filter(locprim__exact=val)
            #print "G :", len(qs)

        if 'locsecond' in get and get['locsecond'] != '':
            val = get['locsecond']
            qs = qs.filter(locsecond__exact=val)

            #print "H :", len(qs)

        if 'final_loc' in get and get['final_loc'] != '':
            val = get['final_loc']
